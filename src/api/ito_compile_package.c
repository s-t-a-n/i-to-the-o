...
{
	return (snprintf(dst, size, "%li",*(long *)var));
}
...
{
	return (snprintf(dst, size, "%lli",*(long long *)var));
}
...
static int	add_data_of_type_to_package(int (*f_type_snprintf)(char *, size_t, void *),
										void *var,
										t_package *package)
{
	int									len;
...
int		add_data_to_package(va_list *args,
		...
		case 'l':
			...
			else if (*(formatstr + 1) == 'l')
			{
				if (*(formatstr + 2) == 'i')
				{
					...
					long long var8 = va_arg(*args, long long);
